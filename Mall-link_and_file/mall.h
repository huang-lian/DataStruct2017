...
void Mall::Free(Appliances * list) {
  Appliances * p = NULL;
  while(NULL != list) {
    p = list;
    list = list->next;
...
void Mall::Stock(void) {
  ...
  const char *msg = "meiygaishanping shihou(y/n)?";
  Product alter_goods;
  char ch;
  ...
  while(ch != 'q' && ch != 'Q') {
    ...
    cout << "该类商品信息如下:\n";
    int cnt = 0;
    for(Appliances *p = alter_list; NULL != p; p->next) {
      cnt++;
      cout << cnt << p->app << endl;
    }
    cout << "同名商品数: " << cnt << endl;
