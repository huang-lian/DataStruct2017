...
- 前置条件:选择了新增
- 后置条件:返回更新
- 正常过程: 提示新增,输入新货物信息,添加,添加成功,显示新加入信息,提示是否继续,输入q,返回更新列表功能.
- 异常过程,可选过程,同创建表单
### 3.8.2 更新-删除商品
- 前置条件: 选择了删除.
- 后置条件:返回更新.
- 正常过程:提示删除,输入删除产品,删除,显示删除结果,提示是否继续,取消,返回更新列表.
- 异常过程:无结果
## 3.8.3 更新-修改商品信息
...
- 前置条件: 选择了结束营业
- 后置条件: 结束程序
- 正常过程: 提示正在结束营业,保存信息表信息,提示保存结果,退出程序.
- 异常过程: 保存失败,提示无法保存,如果继续结束营业,可能会丢失信息, 选择取消,继续运行程序;
- 异常过程2:保存失败,警告提示,选择继续退出,退出.
